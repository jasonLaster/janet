    ...
    async function fetchPdfData() {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error ||
              `Failed to fetch PDF data (Status: ${response.status})`
          );
        }
        ...
        setError(
          error instanceof Error ? error.message : "Failed to load PDF data"
        );
      } finally {
        setLoading(false);
      }
  ...
  if (useFallback) {
    return (
      <div className="h-full w-full">
        <FallbackPdfViewer pdfUrl={pdfData.url} pdfTitle={pdfData.title} />
      </div>
    );
  }
